...
- Вход: `fio`.
- Выход: `ID`, `sFullName`, `sSerialNo`, **`sInventNumber`** (каноническое имя в `itamPortfolio`, см. artibut_model), при необходимости дублируйте старое `sInventoryNo` для совместимости, `lUserId.sFullName`, `bInventoried`, `dtInvent`.
...
- Реализация в репозитории: **`ATRACKER_SERVICE_4_AssetFind.cs`** — скопируйте код в карточку сервиса с ID **4** (или тем, что указан в `config` как сервис карточки актива).
...
1. `CreateAssetAddRequest` (POST)  
   Код-шаблон: `ATRACKER_SERVICE_CreateAssetAddRequest.cs`

2. `AttachRequestDocument` (POST)  
   Код-шаблон: `ATRACKER_SERVICE_AttachRequestDocument.cs`

3. `GetAssetAddRequestState` (GET)  
   Код-шаблон: `ATRACKER_SERVICE_GetAssetAddRequestState.cs`

ТЗ на мастер A-Tracker для обработки заявок:  
`ATRACKER_MASTER_AssetAdd_TZ.md`

Готовый transitionScript шага «Завершить» в мастере:
`ATRACKER_MASTER_AssetAdd_Finalize.cs`
